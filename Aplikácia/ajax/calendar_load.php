...
for ( $i = 1; $i <= $num_of_days; $i++ )
  $days[$i] = new Day($i, $month, $year, $my_account->id);
...
for ( $i = 1; $i <= $num_of_days; $i++ ) {
  ...
  if ( $i == $num_of_days || ( $days[$i]->type != 3 && $days[$i]->day_of_week <= 5 && !$days[$i]->holiday ) ){
    if ( $start && $end ) {
      ...
      $holidays_data .= print_calendar_holidays_value ( $my_account, $from, $to, $count );
      $start = $end = $count = 0;
    }
  }
}
...
echo print_calendar_holiday_script ( $my_account->holidays_spend );
echo print_calendar_table($year, $month, $year_minus, $month_minus, $year_plus, $month_plus, $data);
echo print_calendar_holidays( $holidays_data );
echo print_calendar_overview($year, $month, $my_account->id);
